  ...
  return (
    <nav className="sticky top-0 z-50 w-full max-w-full border-b border-zinc-800 bg-zinc-950/95 pt-[calc(env(safe-area-inset-top,0px)+2rem)] backdrop-blur sm:pt-[calc(env(safe-area-inset-top,0px)+0.5rem)]">
      <div className="box-border flex w-full min-w-0 max-w-full flex-wrap items-center justify-center gap-x-2 gap-y-3 px-3 py-4 sm:mx-auto sm:max-w-6xl sm:justify-between sm:gap-x-2 sm:gap-y-2 sm:px-6 sm:py-3">
        <div className="flex w-full min-w-0 flex-wrap items-center justify-center gap-2.5 sm:flex-1 sm:justify-end sm:gap-2">
          {session === undefined ? (
            <Loader2 className="h-7 w-7 animate-spin text-zinc-500 sm:h-5 sm:w-5" aria-label="Cargando menú" />
          ) : (
            links.map((link) => {
              const isActive =
                pathname === link.href ||
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`inline-flex min-h-14 shrink-0 items-center justify-center rounded-xl border px-5 py-3 text-base font-semibold leading-tight tracking-tight transition sm:min-h-0 sm:rounded-lg sm:px-3 sm:py-1.5 sm:text-sm sm:font-medium ${
                    isActive
