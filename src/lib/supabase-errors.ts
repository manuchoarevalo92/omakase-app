...
export function formatPostgrestError(error: {
  ...
  if (
    ...
    bits.push(
      "El CHECK de la columna proveedor en Supabase está desactualizado (faltan proveedores nuevos en la lista permitida). En Dashboard → SQL Editor ejecutá el SQL del proveedor en supabase/ (p. ej. proveedores-makro-bbq-flavour.sql) y volvé a cargar la página."
    );
  }
  if (
